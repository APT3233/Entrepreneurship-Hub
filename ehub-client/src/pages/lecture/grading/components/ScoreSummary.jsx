    ...
    <div className="rounded-2xl border border-gray-100 bg-white p-5 shadow-sm">
      ...
      {hasProblems ? (
        <div className="mt-4 rounded-xl border border-amber-100 bg-amber-50 p-3 text-sm text-amber-700">
          {invalidCount > 0 ? <p>{t("lecturer.gradingPage.summary.invalidScores", { count: invalidCount })}</p> : null}
          {missingScoreCount > 0 ? <p>{t("lecturer.gradingPage.summary.missingScores", { count: missingScoreCount })}</p> : null}
          {missingFeedbackCount > 0 ? <p>{t("lecturer.gradingPage.summary.missingFeedback", { count: missingFeedbackCount })}</p> : null}
        </div>
      ) : (
