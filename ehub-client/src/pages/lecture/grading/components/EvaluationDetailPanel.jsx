      ...
      <div className="flex flex-col gap-3 border-b border-gray-100 pb-4 sm:flex-row sm:items-start sm:justify-between">
        <div>
          <div className="flex items-center gap-2">
            <h3 className="text-lg font-bold text-gray-900">Evaluation hiện tại</h3>
            <EvaluationStatusBadge value={evaluation.status} />
          </div>
          <p className="mt-1 text-sm text-gray-500">Người chấm: {evaluation.evaluator_name || evaluation.evaluator_id || "—"}</p>
        </div>
        <div className="text-left sm:text-right">
          <p className="text-3xl font-bold text-gray-900">{Number(evaluation.total_score || 0).toFixed(2)}</p>
          <DateTimeCell value={evaluation.evaluated_at || evaluation.updated_at} />
        </div>
      </div>

      <div className="mt-4">
        <p className="text-xs font-semibold uppercase text-gray-400">Feedback tổng</p>
        ...
        {(evaluation.scores || []).map((score) => (
          <div key={score.id || score.criterion_id} className="rounded-xl border border-gray-100 bg-gray-50 p-4">
            <div className="flex items-start justify-between gap-3">
              <div>
                <p className="font-semibold text-gray-900">{score.criterion_name || `Criterion ${score.criterion_id}`}</p>
                <p className="mt-1 text-xs text-gray-500">Max {Number(score.max_score || 0)}</p>
              </div>
