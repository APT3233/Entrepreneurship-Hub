...
export default function NominateStartupPage() {
  ...
  if (loading) return <div className="rounded-2xl bg-surface p-8 text-center text-sm text-slate-400">{t("common.loading")}</div>;

  return (
    <div className="space-y-5">
      <button type="button" onClick={() => navigate(-1)} className="inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-surface px-3 py-2 text-sm font-bold text-slate-700 hover:bg-slate-50"><ArrowLeft size={16} /> {t("common.back")}</button>
      ...
      <form onSubmit={submit} className="rounded-2xl border border-slate-100 bg-surface p-5 shadow-sm">
        ...
        <div className="mt-5 flex justify-end"><button disabled={saving} className="inline-flex items-center gap-2 rounded-xl bg-accent px-4 py-2 text-sm font-bold text-white hover:bg-accent-hover disabled:opacity-50"><Send size={16} /> {t("lecturer.incubationPage.submitNomination")}</button></div>
      </form>
    </div>
  );
}
