...
export default function StartupDetailPage() {
  ...
  const milestoneColumns = useMemo(() => [
    ...
    { key: "actions", label: "", render: (row) => <div className="flex justify-end gap-1"><button type="button" onClick={() => { setMilestoneForm({ ...row, milestone_date: String(row.milestone_date || "").slice(0, 10) }); setMilestoneModal(true); }} className="rounded-lg p-2 text-accent hover:bg-accent-bg">{t("admin.ecosystem.columns.edit")}</button><button type="button" onClick={() => setDeleteMilestone(row)} className="rounded-lg p-2 text-rose-600 hover:bg-rose-50"><Trash2 size={16} /></button></div> },
  ], [t]);
  ...
  const progressColumns = useMemo(() => [
    ...
    { key: "actions", label: "", width: 130, render: (row) => <div className="flex justify-end gap-1"><button type="button" onClick={() => { setProgressForm({ ...row, progress_date: String(row.progress_date || "").slice(0, 10) }); setProgressModal(true); }} className="rounded-lg p-2 text-accent hover:bg-accent-bg">{t("admin.ecosystem.columns.edit")}</button><button type="button" onClick={() => setDeleteProgressTarget(row)} className="rounded-lg p-2 text-rose-600 hover:bg-rose-50"><Trash2 size={16} /></button></div> },
  ], [t]);
  ...
  const awardColumns = useMemo(() => [
    ...
    { key: "evidence_url", label: t("admin.ecosystem.columns.evidence"), render: (row) => row.evidence_url ? <a href={row.evidence_url} target="_blank" rel="noreferrer" className="font-bold text-accent hover:underline">{t("admin.ecosystem.columns.open")}</a> : "-" },
  ], [t]);
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex gap-2 overflow-x-auto rounded-card border border-border bg-surface p-2">
        {tabs.map((tab) => <button key={tab} type="button" onClick={() => openTab(tab)} className={`whitespace-nowrap rounded-xl px-3 py-2 text-sm font-bold ${activeTab === tab ? "bg-accent-bg text-accent" : "text-slate-500 hover:bg-slate-50"}`}>{t(`admin.ecosystem.startupDetail.tabs.${tab}`)}</button>)}
      </div>
      ...
      {activeTab === "pipeline" ? <Panel title={t("admin.ecosystem.startupDetail.panels.pipeline")}><form onSubmit={moveStage} className="mb-4 grid gap-3 rounded-xl bg-slate-50 p-3 md:grid-cols-[1fr_150px_150px_1fr_auto]"><Field label={t("admin.ecosystem.startupDetail.fields.stage")}><SelectField value={moveForm.stage_id} onChange={(stage_id) => setMoveForm((prev) => ({ ...prev, stage_id }))} options={stageOptions} /></Field><Field label={t("admin.ecosystem.columns.action")}><SelectField value={moveForm.action} onChange={(action) => setMoveForm((prev) => ({ ...prev, action }))} options={["moved", "on_hold", "resumed", "graduated", "archived", "rejected"].map((value) => ({ value, label: t(`status.${value}`) || value }))} /></Field><Field label={t("admin.ecosystem.startupDetail.fields.entryStatus")}><SelectField value={moveForm.status} onChange={(status) => setMoveForm((prev) => ({ ...prev, status }))} options={["active", "on_hold", "completed", "archived"].map((value) => ({ value, label: t(`status.${value}`) || value }))} /></Field><Field label={t("admin.ecosystem.columns.reason")}><input className={inputClass} value={moveForm.reason || ""} onChange={(e) => setMoveForm((prev) => ({ ...prev, reason: e.target.value }))} /></Field><button disabled={saving} className="mt-5 rounded-xl bg-accent px-4 py-2 text-sm font-bold text-white disabled:opacity-50">{t("admin.ecosystem.columns.move")}</button></form><AdminTable columns={historyColumns} rows={startup.pipeline_history || []} emptyText={t("admin.ecosystem.startupDetail.empty.pipelineHistory")} /></Panel> : null}
      {activeTab === "progress" ? <Panel title={t("admin.ecosystem.startupDetail.panels.progressTimeline")} actions={<button type="button" onClick={() => { setProgressForm({ update_title: "", update_type: "product", progress_date: today(), visibility: "internal", update_content: "" }); setProgressModal(true); }} className="inline-flex items-center gap-2 rounded-xl bg-accent px-3 py-2 text-sm font-bold text-white"><Plus size={16} /> {t("admin.ecosystem.common.add")}</button>}><AdminTable columns={progressColumns} rows={progressRows} emptyText={t("admin.ecosystem.startupDetail.empty.progress")} /></Panel> : null}
      {activeTab === "metrics" ? <Panel title={t("admin.ecosystem.startupDetail.panels.metricsSnapshots")} actions={<button type="button" onClick={() => { setMetricsForm({ snapshot_date: today(), product_stage: startup.product_stage || "idea", revenue_currency: "VND" }); setMetricsModal(true); }} className="inline-flex items-center gap-2 rounded-xl bg-accent px-3 py-2 text-sm font-bold text-white"><Plus size={16} /> {t("admin.ecosystem.common.add")}</button>}><AdminTable columns={metricsColumns} rows={metricsRows} emptyText={t("admin.ecosystem.startupDetail.empty.metrics")} /></Panel> : null}
      {activeTab === "support" ? <div className="space-y-5"><Panel title={t("admin.ecosystem.startupDetail.panels.supportNeeds")} actions={<button type="button" onClick={() => { setSupportNeedForm({ need_type: "business", priority: "normal", title: "", description: "" }); setSupportNeedModal(true); }} className="inline-flex items-center gap-2 rounded-xl bg-accent px-3 py-2 text-sm font-bold text-white"><Plus size={16} /> {t("admin.ecosystem.common.add")}</button>}><AdminTable columns={supportNeedColumns} rows={supportNeeds} emptyText={t("admin.ecosystem.startupDetail.empty.supportNeeds")} /></Panel><Panel title={t("admin.ecosystem.startupDetail.panels.supportActivities")} actions={<button type="button" onClick={() => { setSupportActivityForm({ activity_type: "mentor_session", activity_date: today(), title: "" }); setSupportActivityModal(true); }} className="inline-flex items-center gap-2 rounded-xl bg-accent px-3 py-2 text-sm font-bold text-white"><Plus size={16} /> {t("admin.ecosystem.common.add")}</button>}><AdminTable columns={supportActivityColumns} rows={supportActivities} emptyText={t("admin.ecosystem.startupDetail.empty.supportActivities")} /></Panel></div> : null}
      {activeTab === "milestones" ? <Panel title={t("admin.ecosystem.startupDetail.panels.milestones")} actions={<button type="button" onClick={() => { setMilestoneForm({ title: "", milestone_type: "product", milestone_date: "" }); setMilestoneModal(true); }} className="inline-flex items-center gap-2 rounded-xl bg-accent px-3 py-2 text-sm font-bold text-white"><Plus size={16} /> {t("admin.ecosystem.common.add")}</button>}><AdminTable columns={milestoneColumns} rows={startup.milestones || []} emptyText={t("admin.ecosystem.startupDetail.empty.milestones")} /></Panel> : null}
      {activeTab === "awards" ? <Panel title={t("admin.ecosystem.startupDetail.panels.awards")} actions={<button type="button" onClick={() => { setAwardForm({ award_name: "", award_type: "winner", awarded_at: today() }); setAwardModal(true); }} className="inline-flex items-center gap-2 rounded-xl bg-accent px-3 py-2 text-sm font-bold text-white"><Plus size={16} /> {t("admin.ecosystem.common.add")}</button>}><AdminTable columns={awardColumns} rows={awards} emptyText={t("admin.ecosystem.startupDetail.empty.awards")} /></Panel> : null}
...
function EvaluationSource({ source }) {
  ...
  return <Panel title={t("admin.ecosystem.startupDetail.panels.evaluationSource")}><div className="space-y-3"><div className="rounded-xl bg-slate-50 p-4"><p className="text-sm font-black text-slate-900">{source.topic || source.group_name}</p><p className="mt-1 text-sm text-slate-500">{source.topic_desc || source.category || t("admin.ecosystem.columns.noTopicDescription")}</p></div><div className="rounded-xl bg-accent-bg p-4 text-sm font-bold text-accent">{t("admin.ecosystem.columns.averageScore")}: {source.average_score ?? "-"}</div><pre className="max-h-72 overflow-auto whitespace-pre-wrap rounded-xl bg-slate-900 p-4 text-xs text-slate-100">{source.feedback_summary || t("admin.ecosystem.columns.noFeedbackSummary")}</pre></div></Panel>;
}
