...
export default function AdminGradeAudit() {
  ...
  const options = useMemo(() => ({
    users: toSelectOptions(lookups.graders, (item) => item.id, (item) => item.full_name || item.email, "Tất cả người dùng"),
    actions: toSelectOptions(lookups.auditActions, (item) => item.action || item, (item) => item.action || item, "Tất cả action"),
  }), [lookups]);
  ...
  return (
    <>
      ...
      >
        <SearchInput value={query.search} onChange={(value) => setFilter("search", value)} placeholder="Action, table, user..." />
        <FilterSelect label="User" value={query.user_id} onChange={(value) => setFilter("user_id", value)} options={options.users} />
        <FilterSelect label="Action" value={query.action} onChange={(value) => setFilter("action", value)} options={options.actions} />
        <FilterSelect label="Table" value={query.table_name} onChange={(value) => setFilter("table_name", value)} options={tableOptions} />
