...
export default function AdminEvaluationResults() {
  ...
  const options = useMemo(() => ({
    ...
    groups: toSelectOptions(lookups.groups, (item) => item.id, (item) => item.group_name || "—", t("lookupAll.groups")),
    graders: toSelectOptions(lookups.graders, (item) => item.id, (item) => item.full_name || item.email, t("lookupAll.graders")),
  }), [lookups, t]);

  const columns = useMemo(() => [
    { key: "group", label: c.group, render: (row) => <span className="font-semibold text-gray-900">{row.group_name || "—"}</span> },
    { key: "topic", label: c.projectTopic, render: (row) => row.topic || "—" },
    { key: "class", label: c.class, render: (row) => row.class_code },
    { key: "semester", label: c.semester, render: (row) => row.semester_code },
    { key: "item", label: c.checkpointAssignment, render: (row) => <div><StatusBadge value={row.source_type} /><p className="mt-1 font-medium text-gray-700">{row.item_title}</p></div> },
    { key: "score", label: c.score, render: (row) => formatScore(row.score, row.max_score) },
    { key: "percentage", label: "%", render: (row) => row.percentage === null || row.percentage === undefined ? "—" : formatPercent(row.percentage) },
    { key: "feedback", label: c.feedback, render: (row) => <span className="line-clamp-2 max-w-[260px]">{row.feedback || "—"}</span> },
    { key: "graded_by", label: c.gradedBy, render: (row) => row.graded_by_name || "—" },
    { key: "graded_at", label: c.gradedAt, render: (row) => formatDate(row.graded_at) },
      ...
      render: (row) => (
        <div className="flex justify-end gap-1">
          <Link to={row.source_type === "checkpoint" ? `/admin/checkpoints/${row.item_id}` : `/admin/assignments/${row.item_id}`}>
            <ActionButton title={t("admin.checkpoint.submissions")}><ExternalLink size={16} /></ActionButton>
          </Link>
          <Link to={`/admin/groups/${row.group_id}`}>
            <ActionButton title={c.group} tone="blue"><ExternalLink size={16} /></ActionButton>
          </Link>
        </div>
      ),
    },
  ...
  return (
    <>
      ...
      >
        <SearchInput value={query.search} onChange={(search) => setQuery((prev) => ({ ...prev, page: 1, search }))} placeholder={t("searchPlaceholders.evaluationResults")} />
        ...
        <label className="flex items-center gap-2 text-sm font-medium text-gray-500">
          {t("admin.evaluationOps.results.scoreRange")}
          <input type="number" min="0" className={smallInputClass} value={query.score_min} onChange={(e) => setQuery((prev) => ({ ...prev, page: 1, score_min: e.target.value }))} placeholder={t("admin.columns.minScore")} />
          <input type="number" min="0" className={smallInputClass} value={query.score_max} onChange={(e) => setQuery((prev) => ({ ...prev, page: 1, score_max: e.target.value }))} placeholder={t("admin.columns.maxScoreShort")} />
        </label>
      </FilterBar>
      <AdminTable columns={columns} rows={rows} loading={loading} error={error} meta={meta} onPageChange={(page) => setQuery((prev) => ({ ...prev, page }))} emptyText={t("admin.evaluationOps.results.empty")} />
