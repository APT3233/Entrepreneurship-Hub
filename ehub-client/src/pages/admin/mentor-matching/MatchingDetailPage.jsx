  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <button onClick={() => navigate(-1)} className="inline-flex items-center gap-2 rounded-xl border border-border bg-surface px-3 py-2 text-sm font-bold text-slate-700"><ArrowLeft size={16} /> {t("admin.mentorMatching.back")}</button>
        <button onClick={generate} className="inline-flex items-center gap-2 rounded-xl bg-accent px-4 py-2 text-sm font-black text-white hover:bg-accent-hover"><Sparkles size={16} /> {t("admin.mentorMatching.generateSuggestions")}</button>
      </div>
      ...
      {suggestions[0] ? <section className="rounded-card border border-border bg-surface p-5"><h3 className="mb-4 text-sm font-black text-slate-900">{t("admin.mentorMatching.topSuggestion")}</h3><SuggestionSummary suggestion={suggestions[0]} /></section> : null}
