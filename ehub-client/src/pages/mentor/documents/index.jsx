...
export default function MentorDocumentsPage() {
  ...
  const columns = useMemo(() => [
    ...
    { key: "file_name", label: t("mentorPortal.documents.file"), render: (row) => <span className="font-medium text-text-primary">{row.file_name}</span> },
    ...
    { key: "actions", label: "", width: 100, render: (row) => <div className="flex justify-end gap-1"><a href={row.file_url} target="_blank" rel="noreferrer" className="rounded-control p-2 text-accent hover:bg-accent-bg"><ExternalLink size={16} /></a><button type="button" onClick={() => setDeleteDoc(row)} className="rounded-control p-2 text-danger-text hover:bg-danger-bg"><Trash2 size={16} /></button></div> },
  ], [t]);
  ...
  return (
    <div className="space-y-4">
      <form onSubmit={uploadDocument} className="grid gap-3 rounded-card border border-border bg-surface p-4 md:grid-cols-[180px_1fr_auto]">
        ...
        <Field label={t("mentorPortal.documents.file")}><input type="file" className="block w-full text-sm text-text-secondary file:mr-3 file:rounded-control file:border-0 file:bg-accent-bg file:px-3 file:py-2 file:text-sm file:font-medium file:text-accent" onChange={(e) => setUpload((prev) => ({ ...prev, file: e.target.files?.[0] || null }))} /></Field>
        <button disabled={saving} className="mt-5 inline-flex items-center justify-center gap-2 rounded-control bg-accent px-4 py-2 text-sm font-medium text-white hover:bg-accent-hover disabled:opacity-50"><Upload size={16} /> {t("mentorPortal.documents.uploadBtn")}</button>
      </form>
