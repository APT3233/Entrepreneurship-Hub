...
export default function StudentStartupProfileDetailPage() {
  ...
  const milestoneColumns = [
    { key: "title", label: t("student.startupProfile.columns.milestone"), render: (row) => <span className="font-black text-slate-900">{row.title}</span> },
  ...
  return (
    <div className="space-y-5">
      <button type="button" onClick={() => navigate("/student/startup-profile")} className="inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm font-bold text-slate-700 hover:bg-slate-50"><ArrowLeft size={16} /> {t("common.back")}</button>
      <StartupHeader startup={startup} />
      <div className="flex gap-2 overflow-x-auto rounded-2xl border border-slate-100 bg-white p-2 shadow-sm">{["profile", "progress", "support"].map((tab) => <button key={tab} type="button" onClick={() => openTab(tab)} className={`whitespace-nowrap rounded-xl px-3 py-2 text-sm font-bold ${activeTab === tab ? "bg-indigo-50 text-indigo-700" : "text-slate-500 hover:bg-slate-50"}`}>{tabLabels[tab]}</button>)}</div>
      ...
      {activeTab === "progress" ? <Panel title={t("student.startupProfile.panels.progress")} actions={<button type="button" onClick={() => setProgressModal(true)} className="inline-flex items-center gap-2 rounded-xl bg-indigo-600 px-3 py-2 text-sm font-bold text-white"><Plus size={16} /> {t("student.startupProfile.actions.add")}</button>}><AdminTable columns={[{ key: "progress_date", label: t("student.startupProfile.columns.date"), render: (row) => formatDate(row.progress_date) }, { key: "update_title", label: t("student.startupProfile.columns.update"), render: (row) => <div><p className="font-black text-slate-900">{row.update_title}</p><p className="mt-1 text-sm text-slate-500">{row.update_content}</p></div> }, { key: "update_type", label: t("admin.ecosystem.common.type"), render: (row) => <StatusBadge value={row.update_type} /> }, { key: "visibility", label: t("admin.ecosystem.common.visibility"), render: (row) => <StatusBadge value={row.visibility} /> }]} rows={progressRows} emptyText={t("student.startupProfile.empty.progress")} /></Panel> : null}
      {activeTab === "support" ? <Panel title={t("student.startupProfile.panels.supportNeeds")} actions={<button type="button" onClick={() => setSupportModal(true)} className="inline-flex items-center gap-2 rounded-xl bg-indigo-600 px-3 py-2 text-sm font-bold text-white"><Plus size={16} /> {t("student.startupProfile.actions.request")}</button>}><AdminTable columns={[{ key: "title", label: t("student.startupProfile.columns.need"), render: (row) => <div><p className="font-black text-slate-900">{row.title}</p><p className="mt-1 text-sm text-slate-500">{row.description}</p></div> }, { key: "need_type", label: t("admin.ecosystem.common.type"), render: (row) => <StatusBadge value={row.need_type} /> }, { key: "priority", label: t("student.startupProfile.columns.priority"), render: (row) => <StatusBadge value={row.priority} /> }, { key: "status", label: t("common.status"), render: (row) => <StatusBadge value={row.status} /> }]} rows={supportNeeds} emptyText={t("student.startupProfile.empty.support")} /></Panel> : null}
