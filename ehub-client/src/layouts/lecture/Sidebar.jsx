...
import {
  Home, BookOpen, Users, ClipboardList, Star, Calendar, PinIcon, PinOff,
} from "lucide-react";
    ...
    <>
      ...
      >
        ...
        <div className="flex items-center gap-3 px-[14px] py-5 border-b border-gray-100 overflow-hidden">
          ...
          >
            <p className="text-[17px] font-bold text-indigo-600 tracking-tight leading-tight">E-HUB</p>
            <p className="text-xs text-gray-400 font-medium mt-0.5">Cổng giảng viên</p>
          </div>
        </div>
              ...
              {({ isActive }) => (
                <>
                  <span className="relative shrink-0">
                    <Icon size={20} className={`transition-colors duration-200 ${isActive ? "text-indigo-500" : "text-slate-400 group-hover:text-slate-600"}`} />
                    {isActive && !isOpen && (
                      <span className="absolute -top-0.5 -right-0.5 w-1.5 h-1.5 rounded-full bg-indigo-500" />
                    )}
                  </span>
                  <span style={{ opacity: isOpen ? 1 : 0, width: isOpen ? "auto" : 0 }} className="transition-all duration-300 ease-in-out overflow-hidden">
                    {label}
                  </span>
                </>
              )}
        ...
        <div className="mt-auto border-t border-gray-100 px-[14px] py-4 flex items-center gap-3 overflow-hidden">
          <button
            ...
            className={`shrink-0 w-8 h-8 rounded-lg flex items-center justify-center transition-all duration-200
              ${pinned ? "bg-indigo-100 text-indigo-600 hover:bg-indigo-200" : "text-slate-400 hover:bg-slate-100 hover:text-slate-600"}`}
          >
          ...
          >
            {({ isActive }) => (
              <>
                ...
                <span className={`text-[9px] font-medium leading-none transition-all duration-200 ${isActive ? "text-indigo-600" : "text-slate-400"}`}>
                  {label}
                </span>
              </>
            )}
